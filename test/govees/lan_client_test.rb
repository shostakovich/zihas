...
class GoveesLanClientTest < ActiveSupport::TestCase
  ...
  test "parse_status returns nil for out-of-range telemetry (never crashes the listener)" do
    payload = JSON.generate("msg" => { "data" => { "onOff" => 1, "brightness" => 254 } })
    assert_nil Govees::LanClient.parse_status(payload)
  end
